...
LeptonFlurry is the ultra-efficient 32-bit counter-based PRNG (non-cryptographic for scientific simulations) that has GPU-friendly 32-bit integers, excellent statistical randomness quality test results, low-footprint implementation (efficient memory usage and small code size), massive parallelism (up to 2⁶⁴ key-based independent streams that each have a counter-based period of 2¹²⁸), no division/modulus/multiplication operators and ultra-fast speed.
...
The `leptonflurry1x32` function generates (from 4 `uint32_t` `counter` integers that must be seeded and 2 `uint32_t` `key` integers that must be seeded) a deterministic pseudorandom `uint32_t` integer as the return value.
...
LeptonFlurry streams yielded the following excellent results from statistical randomness quality tests.
